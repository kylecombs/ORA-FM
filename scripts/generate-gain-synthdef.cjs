#!/usr/bin/env node
// ════════════════════════════════════════════════════════════
//  Generate ora_gain.scsyndef
//
//  A stereo gain module with Lag.kr for click-free parameter
//  changes. Reads stereo audio from in_bus, multiplies by a
//  lagged amp parameter, and writes to out_bus.
//
//  Equivalent SuperCollider source:
//    SynthDef(\ora_gain, {
//        var amp = Lag.kr(\amp.kr(1), 0.005);
//        var sig = In.ar(\in_bus.kr(0), 2);
//        Out.ar(\out_bus.kr(0), sig * amp);
//    });
//
//  Usage:  node scripts/generate-gain-synthdef.cjs
// ════════════════════════════════════════════════════════════

const fs = require('fs');
const path = require('path');

function buildGainSynthDef() {
  const parts = [];
  ...
  // UGen 0: Control.kr → 3 outputs (in_bus, out_bus, amp)
  ...
  writeInt8(1);            // output 2 rate: control (amp)

  // UGen 1: Lag.kr(amp, 0.005) → 1 output
  writePString('Lag');
  writeInt8(1);            // rate: control
  writeInt32(2);           // 2 inputs
  writeInt32(1);           // 1 output
  writeInt16(0);           // special index
  // input 0: amp from Control
  writeInt32(0);           // src: UGen 0
  writeInt32(2);           // output 2 (amp)
  ...
  // UGen 3: BinaryOpUGen.ar(L * lagged_amp) → 1 output
  ...
  // input 1: lagged amp from Lag
  writeInt32(1);           // src: UGen 1
  writeInt32(0);           // output 0
  writeInt8(2);            // output 0 rate: audio

  // UGen 4: BinaryOpUGen.ar(R * lagged_amp) → 1 output
  writePString('BinaryOpUGen');
  writeInt8(2);            // rate: audio
  writeInt32(2);           // 2 inputs
  writeInt32(1);           // 1 output
...
// ── Write the file ──
const data = buildGainSynthDef();
const outPath = path.join(__dirname, '..', 'public', 'supersonic', 'synthdefs', 'ora_gain.scsyndef');
fs.writeFileSync(outPath, data);
console.log(`Wrote ora_gain.scsyndef (${data.length} bytes) → ${outPath}`);
